...
void votemajoritaire(const vector<participant>& vParticipant,
    ...
    int nbGroupes = 24 ;// nombre de groupe

    for (int groupe = 0; groupe < nbGroupes; groupe += 1) {
        vector<int> votesGroupe(4, 0); // Compteur de votes pour le groupe actuel
        ...
        int gagnantGroupe = 0;
        int maxVotes = votesGroupe[0];

        for (int glace = 1; glace < 4; glace +=1) {
            if (votesGroupe[glace] > maxVotes) {
                maxVotes = votesGroupe[glace];
                gagnantGroupe = glace;
            }
        }
...
int main()
{
    ...
    for (unsigned i = 0; i < 4; i+=1) {
        ...
        double scoreMajoritaire = (pointsMajoritaires[i] / 24.0) * 50.0;
        //
        double scoreProportionnel = (voteproportionnel[i] / 120.0) * 50.0;
        scoresFinaux[i] = scoreMajoritaire + scoreProportionnel;

        cout << vGlacePref[i] << " : " << scoresFinaux[i] << " points ("
             << scoreMajoritaire << " majoritaire + "
             << scoreProportionnel << " proportionnel)" << endl;
    }
