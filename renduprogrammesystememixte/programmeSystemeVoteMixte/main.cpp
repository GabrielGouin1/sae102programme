...
void votemajoritaire(const vector<participant>& vParticipant, const vector<string>& vGlacePref, vector <int>& pointsMajoritaires){
    int nombreGroupe = 24;// nombre de groupe
    for(int groupe = 0; groupe < nombreGroupe; groupe +=1){
        vector<int> voteGroupe(4); //compteur de votes pour le groupe dont c'est l'itération$
        // on compte alors les votes eds différents participants du groupe
        for (int nparticipant = 0; nparticipant < 5; nparticipant+=1) {
            int locparticipant = groupe*5 + nparticipant;
...
int main()
{
    ...
    //Votes proporionnel
    vector<int> voteproportionnel(4,0);
